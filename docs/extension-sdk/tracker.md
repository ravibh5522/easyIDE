...
| Icon themes | in-progress | `workbench.iconTheme` + Settings picker; `ActiveIconTheme`/`IconThemeFile` (fileNames > multi-part extensions > languageIds > file; folder names; `light` section), PNG via `BitmapFactory` + LRU (`SettingsPolicy.ICON_CACHE_ENTRIES`); explorer uses it through `LocalFileIcons`. SVG and font icons skipped and logged (customization.md open issue 1) |
