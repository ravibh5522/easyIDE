...
| Bootstrap tarball sourcing (download) | done | `RootfsProvisioner` downloads ubuntu-base with progress, resumable-safe (`.part` then rename), device-wide image cache. **No checksum verification yet** |
...
- **Rootfs image downloads are not checksum-verified.** The URL is HTTPS, but a truncated or tampered archive would only surface as a confusing extraction failure. Add a published SHA256 check before shipping.
