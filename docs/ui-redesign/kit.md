...
| Screenshot | Roborazzi 1.75.0 (Apache-2.0, updated 2026-09-21, needs Robolectric 4.17, MIT) runs in the JVM unit-test task; compatibility with AGP 9.3.1 and Kotlin 2.4.10 is **unverified**, so a one-day spike on the kit gallery comes first (also verify that the bundled Geist fonts load under Robolectric). Paparazzi is not adopted (only an alpha is current). Dropshots 0.6.0 is the on-device fallback |
