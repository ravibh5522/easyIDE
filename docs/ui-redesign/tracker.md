...
| Settings on the shell (categories + pages + Appearance + Layout) | in-progress | built on the kit, not seen on a device: `SettingsPanel` (search, layer control, safe-mode banner, categories or results) and `SettingsPage(category)` (generic schema rows with modified dot, layer badge, reset, protected and per-language handling, search results page), new Appearance page (live preview, accent swatches and hex with contrast and hue feedback, all `appearance.*` rows) and Layout page (`shell.*` keys in `ui/props/ShellSettingsSchema`, up/down reorder, hide, pin, panel placement); Keyboard, Language servers, Sandbox, Advanced are pages; all Settings dialogs on `KitDialog`. Open: `ShellSettingsSchema.all` is not yet added to `SettingsSchema.all` (one line, kept out to avoid a merge conflict), no shell wiring (`SettingsScreen` adapter stays until the shell hosts the panel and pages), `GitCredentialDialog` in workspace/git is still stock, no screenshot goldens |
