...
| **R0 Foundations** | | |
| Import-ratchet test + allowlist | done | `ImportRatchetTest`; allowlist `ui-ratchet-allowlist.txt` = 29 files, 87 imports (was 55 files, 202 imports before the shell hosted Home, Settings and Extensions; Home, Settings, Extensions and the old screens left it); fails on growth, new offenders and un-lowered improvements. `GitCredentialDialog` (workspace/git) stays listed until R3; ThemePicker's dynamic colour-scheme imports are not on the banned list |
