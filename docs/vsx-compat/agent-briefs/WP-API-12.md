# WP-API-12 - `FeatureSource` + `DocumentSelector` router refactor

Generated from [roadmap.md](../roadmap.md) row 44 by `tools/vsx-audit/gen-tracker-briefs.mjs`. Ready-to-run prompt for an
autonomous implementation agent. Milestone M2. Effort M / 5. Risk Med. Owner role: lsp-router.

## Prompt

You are the **lsp-router** agent implementing **WP-API-12 (`FeatureSource` + `DocumentSelector` router refactor)** for easyIDE's VS Code extension compatibility
program. Work in your own git worktree on a branch named `wp/wp-api-12`, one work package per branch.
...
1. Read first: `.claude/CLAUDE.md` and [CONTRIBUTING.md](../../../CONTRIBUTING.md) (600-line file limit, no hardcoding,
   ...
   and the matrix rows that name WP-API-12 in [matrix/](../matrix/).
2. Dependencies that must be Done in [tracker.md](../tracker.md) before you start: none.
3. Scope: exactly what the source docs above assign to WP-API-12; anything else goes to the lead as a note, not into this branch.
   Respect the ownership boundaries and merge order in [roadmap.md](../roadmap.md) section 4.
4. Acceptance (must be demonstrated, with the command output or device evidence in the PR): existing LSP tests green; `editor.defaultFormatter="esbenp.prettier-vscode"` selection test
...
6. In the same PR: set WP-API-12 to Done (or In progress with a note) in [tracker.md](../tracker.md), update the matrix rows it
