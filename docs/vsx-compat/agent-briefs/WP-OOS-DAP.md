# WP-OOS-DAP - Stub slice: DebugService N+R

Generated from [roadmap.md](../roadmap.md) row 9 by `tools/vsx-audit/gen-tracker-briefs.mjs`. Ready-to-run prompt for an
autonomous implementation agent. Milestone M1. Effort S / 2. Risk Low. Owner role: exthost-js.

## Prompt

You are the **exthost-js** agent implementing **WP-OOS-DAP (Stub slice: DebugService N+R)** for easyIDE's VS Code extension compatibility
program. Work in your own git worktree on a branch named `wp/wp-oos-dap`, one work package per branch.
...
1. Read first: `.claude/CLAUDE.md` and [CONTRIBUTING.md](../../../CONTRIBUTING.md) (600-line file limit, no hardcoding,
   ...
   and the matrix rows that name WP-OOS-DAP in [matrix/](../matrix/).
2. Dependencies that must be Done in [tracker.md](../tracker.md) before you start: WP-HOST-12.
3. Scope: exactly what the source docs above assign to WP-OOS-DAP; anything else goes to the lead as a note, not into this branch.
   Respect the ownership boundaries and merge order in [roadmap.md](../roadmap.md) section 4.
4. Acceptance (must be demonstrated, with the command output or device evidence in the PR): `$registerDebugTypes` accepted; `$startDebugging` -> false + banner; `-32010 {track:"OOS-DAP"}` on use
...
6. In the same PR: set WP-OOS-DAP to Done (or In progress with a note) in [tracker.md](../tracker.md), update the matrix rows it
