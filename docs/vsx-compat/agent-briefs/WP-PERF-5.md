# WP-PERF-5 - UI-link throughput (queues, coalescing, side channel)

Generated from [roadmap.md](../roadmap.md) row 65 by `tools/vsx-audit/gen-tracker-briefs.mjs`. Ready-to-run prompt for an
autonomous implementation agent. Milestone M2. Effort M / 5. Risk Med. Owner role: perf.

## Prompt

You are the **perf** agent implementing **WP-PERF-5 (UI-link throughput (queues, coalescing, side channel))** for easyIDE's VS Code extension compatibility
program. Work in your own git worktree on a branch named `wp/wp-perf-5`, one work package per branch.
...
1. Read first: `.claude/CLAUDE.md` and [CONTRIBUTING.md](../../../CONTRIBUTING.md) (600-line file limit, no hardcoding,
   ...
   and the matrix rows that name WP-PERF-5 in [matrix/](../matrix/).
2. Dependencies that must be Done in [tracker.md](../tracker.md) before you start: WP-HOST-5.
3. Scope: exactly what the source docs above assign to WP-PERF-5; anything else goes to the lead as a note, not into this branch.
   Respect the ownership boundaries and merge order in [roadmap.md](../roadmap.md) section 4.
4. Acceptance (must be demonstrated, with the command output or device evidence in the PR): B20-B22 met under 10k decorations/s, 5k-file diagnostics, 50k-node tree storm
...
6. In the same PR: set WP-PERF-5 to Done (or In progress with a note) in [tracker.md](../tracker.md), update the matrix rows it
