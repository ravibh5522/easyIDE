# WP-SEC-18 - Legal review package for ADR 0034 (external lead time)

Generated from [roadmap.md](../roadmap.md) row 29 by `tools/vsx-audit/gen-tracker-briefs.mjs`. Ready-to-run prompt for an
autonomous implementation agent. Milestone M1 (start). Effort S / 2. Risk High. Owner role: security.

## Prompt

You are the **security** agent implementing **WP-SEC-18 (Legal review package for ADR 0034 (external lead time))** for easyIDE's VS Code extension compatibility
program. Work in your own git worktree on a branch named `wp/wp-sec-18`, one work package per branch.
...
1. Read first: `.claude/CLAUDE.md` and [CONTRIBUTING.md](../../../CONTRIBUTING.md) (600-line file limit, no hardcoding,
   ...
   and the matrix rows that name WP-SEC-18 in [matrix/](../matrix/).
2. Dependencies that must be Done in [tracker.md](../tracker.md) before you start: none.
3. Scope: exactly what the source docs above assign to WP-SEC-18; anything else goes to the lead as a note, not into this branch.
   Respect the ownership boundaries and merge order in [roadmap.md](../roadmap.md) section 4.
4. Acceptance (must be demonstrated, with the command output or device evidence in the PR): counsel sign-off recorded in ADR 0034; no compliance claim before
...
6. In the same PR: set WP-SEC-18 to Done (or In progress with a note) in [tracker.md](../tracker.md), update the matrix rows it
