# WP-SEC-5 - Host launch hygiene (env allow-list, no token, kill-on-exit)

Generated from [roadmap.md](../roadmap.md) row 5 by `tools/vsx-audit/gen-tracker-briefs.mjs`. Ready-to-run prompt for an
autonomous implementation agent. Milestone M1. Effort S / 2. Risk Low. Owner role: security.

## Prompt

You are the **security** agent implementing **WP-SEC-5 (Host launch hygiene (env allow-list, no token, kill-on-exit))** for easyIDE's VS Code extension compatibility
program. Work in your own git worktree on a branch named `wp/wp-sec-5`, one work package per branch.
...
1. Read first: `.claude/CLAUDE.md` and [CONTRIBUTING.md](../../../CONTRIBUTING.md) (600-line file limit, no hardcoding,
   ...
   and the matrix rows that name WP-SEC-5 in [matrix/](../matrix/).
2. Dependencies that must be Done in [tracker.md](../tracker.md) before you start: none.
3. Scope: exactly what the source docs above assign to WP-SEC-5; anything else goes to the lead as a note, not into this branch.
   Respect the ownership boundaries and merge order in [roadmap.md](../roadmap.md) section 4.
4. Acceptance (must be demonstrated, with the command output or device evidence in the PR): ST-VSX-01 on device; ST-12/13 extended to the host
...
6. In the same PR: set WP-SEC-5 to Done (or In progress with a note) in [tracker.md](../tracker.md), update the matrix rows it
