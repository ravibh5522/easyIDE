# WP-UI-8 - Webview foundation (ADR 0033)

Generated from [roadmap.md](../roadmap.md) row 86 by `tools/vsx-audit/gen-tracker-briefs.mjs`. Ready-to-run prompt for an
autonomous implementation agent. Milestone M4. Effort L / 15. Risk High. Owner role: ui-webview.

## Prompt

You are the **ui-webview** agent implementing **WP-UI-8 (Webview foundation (ADR 0033))** for easyIDE's VS Code extension compatibility
program. Work in your own git worktree on a branch named `wp/wp-ui-8`, one work package per branch.
...
1. Read first: `.claude/CLAUDE.md` and [CONTRIBUTING.md](../../../CONTRIBUTING.md) (600-line file limit, no hardcoding,
   ...
   and the matrix rows that name WP-UI-8 in [matrix/](../matrix/).
2. Dependencies that must be Done in [tracker.md](../tracker.md) before you start: WP-UI-1, WP-API-21.
3. Scope: exactly what the source docs above assign to WP-UI-8; anything else goes to the lead as a note, not into this branch.
   Respect the ownership boundaries and merge order in [roadmap.md](../roadmap.md) section 4.
4. Acceptance (must be demonstrated, with the command output or device evidence in the PR): ui-webviews.md §8 T1-T9 on device; `WebviewResourceServerTest`
...
6. In the same PR: set WP-UI-8 to Done (or In progress with a note) in [tracker.md](../tracker.md), update the matrix rows it
