...
internal fun DialogText(text: String, muted: Boolean = false, modifier: Modifier = Modifier) {
    ...
    BasicText(text, modifier.padding(top = Kit.space.xs), style = Kit.text.body.copy(color = color))
}
...
internal fun DialogMono(text: String, modifier: Modifier = Modifier) {
    BasicText(text, modifier.padding(top = Kit.space.xs), style = Kit.text.monoSmall.copy(color = Kit.colors.plainText))
}
