...
private fun Fact(label: String, value: String) {
    KitTwoColumnRow(label) { MonoText(value) }
}
...
internal fun DetailsTab(row: ExtensionRow, lines: List<InspectorLine>, actions: PageActions) {
    ...
    KitSection(stringResource(R.string.extui_section_details), description = trustText(row.pkg.source), collapsible = true) {
        Fact(stringResource(R.string.extui_fact_publisher), row.id.substringBefore('.'))
        d?.license?.let { Fact(stringResource(R.string.extui_fact_license), it) }
        ...
        Fact(stringResource(R.string.extui_fact_scope), scope)
        d?.categories?.takeIf { it.isNotEmpty() }?.let { Fact(stringResource(R.string.extui_fact_categories), it.joinToString()) }
    }
...
internal fun VersionsTab(row: ExtensionRow, item: ExtensionListItem, updateTo: String?, actions: PageActions) {
    KitSection(stringResource(R.string.extui_section_versions), collapsible = true, description = if (actions.onRollback == null && actions.onUpdate == null) stringResource(R.string.extui_versions_none) else null) {
        Fact(stringResource(R.string.extui_version_installed), item.version)
