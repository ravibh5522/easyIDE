...
fun SamplePacksDialog(samples: List<SampleInfo>, onInstall: (String) -> Unit, onDismiss: () -> Unit) {
    KitDialog(
        ...
        BasicText(stringResource(R.string.extui_sample_hint), style = Kit.type.bodySmall.copy(color = Kit.colors.textMuted))
        if (samples.isEmpty()) {
            BasicText(stringResource(R.string.extui_sample_none), style = Kit.type.bodyMedium.copy(color = Kit.colors.plainText))
        } else {
            KitGroup {
