...
sealed interface ShellAction {
    ...
    /** Opens [placement] showing [container] (or what it already shows); unlike [SelectNav] it never collapses an open panel. */
    data class ShowPanel(val placement: Placement, val container: String? = null) : ShellAction
...
object ShellReducer {
    ...
    fun reduce(state: ShellState, action: ShellAction, env: ShellEnv): ShellState {
        val next = when (action) {
            ...
            is ShellAction.ShowPanel -> onLayout(state) { it.show(action.placement, state.rule, action.container) }
