// Word Count (AssemblyScript sample): `wasm-word-count-as.wordCount` shows the word count of the active editor.
...
class Ext extends Extension {
  activate(info: string): string | null {
    const r = call("commands.register", '{"command":"wasm-word-count-as.wordCount"}');
    return r.ok ? null : r.message;
  }

  command(name: string, args: string): Reply {
    if (name != "wasm-word-count-as.wordCount") return Reply.notFound(name);
