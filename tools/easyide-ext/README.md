...
| `validate [dir or .easyext] [--strict] [--engine <ver>]` | Layout and size limits, the app's manifest pipeline (schema, references, when-clauses, capability audit, scope, content, and for UI points the view files, component catalog, limits, ids, titles and icon rules), the app's static WASM check, and with `--strict` registry readiness (README, LICENSE, SPDX `license`, icon size) with warnings failing. |
...
| `test [dir] [--filter <glob>]` | Replays `test/` scenarios with the app's action runner on a recording host with fakes (L1; WASM commands are not executed). `expect.views` also renders a view or document with the app's own view planner and asserts on the tree (component types, texts, row counts); see [views.md](../../docs/extension-sdk/views.md#8-testing-without-a-device). |
